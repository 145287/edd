...
class StrainResourceTests(APITestCase):
    ...
    def _enforce_study_strain_read_access(self, url, is_list, strain_in_study=True):
        ...
        # enforce access denied behavior for the strain detail -- permission denied
            ...
            self._require_authenticated_get_access_denied(url, self.staff_strain_creator)
...
# class StudyResourceTests(APITestCase):
#     """
...
#                                       is_staff=True)
#
#         cls.staff_study_user = _create_user(username='staff.study.user',
#                                             email='staff.study@localhost',
#                                             is_staff=True,
#                                             manage_perms=(cls.add_study_permission,
#                                                            cls.change_study_permission,
#                                                            cls.delete_study_permission))
#
...
#                                        permission_type=StudyPermission.WRITE)
#
#     def _enforce_study_read_access(self, url, is_list):
#         """
#         A helper method that does the work to test permissions for both list and individual study
#         GET access.
#         :param resource_in_study: True if the provided URL references a strain in our test study,
#         False if the strain isn't in the test study, and should only be visible to
#         superusers/managers.
#         """
#
...
#             self._require_authenticated_get_access_denied(url, self.staff_user)
#
#         # test that an 'admin' user can access study data even without the write privilege
#         self._require_authenticated_get_access_allowed(url, self.superuser)
#
#         # test that 'staff' users with any study mutator privileges have implied read permission
#         self._require_authenticated_get_access_denied(url, self.staff_study_creator)
#         self._require_authenticated_get_access_allowed(url, self.staff_study_changer)
#         self._require_authenticated_get_access_allowed(url, self.staff_study_deleter)
#         self._require_authenticated_get_access_allowed(url, self.staff_study_user)
#
#     def test_strain_uuid_pattern_match(self):
#         # TODO: test pattern matching for UUID's. had to make code changes during initial testing
#         # to enforce matching for UUID's returned by EDD's REST API, which is pretty weird after
...
#             'base_study_url': STRAINS_RESOURCE_URL,
#             'pk': study.pk,
#         }
#
#         TODO: enforce study write access
#
#         # unauthenticated user and unprivileged users get 403
#         self.client.logout()
#         response = self.client.delete(url)
...
#
#         # verify that strain change permission doesn't allow addition of new strains
#         self._require_authenticated_post_access_denied(_URL, self.staff_study_changer, post_data)
#
#         # verify that an administrator can create a strain
#         self._require_authenticated_post_access_allowed(_URL,
#                                                         self.superuser,
